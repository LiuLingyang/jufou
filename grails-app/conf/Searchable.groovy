...
//			"${userHome}/.grails/projects/${appName}/searchable-index/${grailsEnv}"
			"${userHome}/.grails/projects/${appName}/searchable-index/${grailsEnv ? grailsEnv : GrailsUtil.getEnvironment()}"
	).absolutePath
	 ...
	 */
	compassSettings = ['compass.engine.analyzer.chinese.type': 'chinese']
...
//			compassConnection = "ram://development-index"
			compassConnection = new File("F:/Projects/Meetup/web/search")
					.absolutePath
//			compassConnection = new File(
//					"${userHome}/.grails/projects/meetup/searchable-index/${grailsEnv ? grailsEnv : GrailsUtil.getEnvironment()}"
//			).absolutePath
		}
	}
	...
	test {
		searchable {
			...
			compassConnection = new File("/mnt/data/search/test")
					.absolutePath
		}
	}
	...
	production {
		searchable {
			...
			bulkIndexOnStartup = true
			compassConnection = new File("/mnt/data/search")
					.absolutePath
		}
	}
}