  ...
  <@module id="40" title="${photo.uploadTime?number_to_datetime?string(\"yyyy-MM-dd\")}"
    ...
    <div class="xmn">
      ...
      <div class="xhw f-cb">
        ...
        <div class="lhw">
          <a class="xit fl" href="${photo.originalURL}" hidefocus="true" data-lightbox="xxx">
            <img src="${photo.originalURL}"/>
          </a>
        </div>
        <div class="xcm js-xflag"></div>
        <div class="js-xflag" style="display:none;"></div>
      </div>
    </div>
  </@module>
</#macro>
...
<#macro groupPhotoShowTemplate album={} photo={}>
  ...
  <textarea name="jst" id="jst-photo-all-list">
    {list xlist as x}
      {if x.id!=cid}
      <a class="xit fl" href="${x.originalURL}" data-lightbox="xxx"></a>
      {/if}
    {/list}
  </textarea>
  </#noparse>
