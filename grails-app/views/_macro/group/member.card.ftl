...
<#macro groupMemberCardRight user={} host={}>
  <div class="fr l-rsd">
    <@module id="82" class="s-srd">
      <div class="img"><img src="${user.portrait!(user.attendee.portrait)!p_portraits}"/></div>
    </@module>
    <@module id="80" title="加入的其他组织" class="s-srd">
      <#if user.attendee.id==host.id>
      <p class="ln0 js-xflag<#if (user.attendee.hidden)!false??> js-show</#if>" style="display:none;">
...
<#-- 成员卡片中间模块 -->
<#macro groupMemberCardMain user={} host={} group={} host_role=0 user_role=0>
  <div class="l-rmc">
    <@module id="81" title="${user.attendee.nickname}" class="t-mdi s-srd">
      <div class="yact js-flag">
        ...
        <#if host.id==user.attendee.id&&(user_role==1||user_role==2||user_role==4)>
