  ...
  <form class="l-lmc w-fom meeting-create">
    ...
    <@module title="创建活动" id="10" class="t-mdi t-mdf s-srd">
      <@groupMeetingCreateBlock title="基本信息" open=true>
        ...
        <div class="sti">
          <p class="itl tip1">活动封面<span class="itp fc06">（必填）</span></p>
          <div class="ptl nxd">
            <label><div id="meeting-cover" class="cvr" style="background-image:url(${meeting.cover!p_meeting_cover});">&nbsp;</div></label>
            <div class="w-loading" id="meeting-cover-loading" style="display:none;">&nbsp;</div>
      ...
      <@groupMeetingCreateBlock title="费用及相关">
        ...
        <div class="sti sti-2 sti-y lbk">
          ...
          <div class="ick signup">
            ...
            <#local dateTime=parseDateTime(meeting.signupStartTime)>
            <input type="text" 
                   readonly="true"  
                   ...
                   value="${dateTime?string("yyyy-MM-dd")}"
                   />
            <@groupMeetingTimeSelect name="signupStartHour" value="${dateTime?string(\"H\")}"/>
            <span class="f-fw1">:</span>
            <@groupMeetingTimeSelect name="signupStartMinute" value="${dateTime?string(\"m\")}" end=59/>
            <a class="abt fc02 js-flag" href="#">设置一个结束时间</a>
          </div>
        </div>
        <#assign signupTime=meeting.signupEndTime!0>
        <div class="sti sti-1 lbk" <#if signupTime==0>style="display:none"</#if>>
           <p class="itl itl-1">
            <span>报名结束时间</span>          
          </p>
          <div class="ick ick-4">
          ...
          <div class="ick  ick-5 signup">
            ...
            <#local dateTime=parseDateTime(meeting.signupEndTime)>
            <input type="text" 
                   readonly="true" 
                   name="signupEndDate" 
                   class="w-mdat txt bd01 w-rd3 f-bg"
                   value="${dateTime?string("yyyy-MM-dd")}"/>
            <@groupMeetingTimeSelect name="signupEndHour" value="${dateTime?string(\"H\")}"/>
            <span class="f-fw1">:</span>
            <@groupMeetingTimeSelect name="signupEndMinute" value="${dateTime?string(\"m\")}" end=59/>
            <a class="abt fc02 js-flag" href="#">移除结束时间</a>
          </div>
        </div>
        <div class="sti lbk">
      ...
      <@groupMeetingCreateBlock title="让更多人知道这次活动" open=editMeeting>
        <div class="sti sti-s lbk">
          <#assign joinPermission=meeting.joinPermission!0>
    ...
    <div class="w-tbar w-tbar-tip bd02 bg01 f-cb">
      ...
      <#else>
      <input name="btn-publish" type="button" class="fr w-rd3 t-btn t-btn-4" value="活动建议"/>
      </#if>
