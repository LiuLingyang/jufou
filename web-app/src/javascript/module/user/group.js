var f = function(){
    ...
    _proModuleGroup.__reset = function(_options){
        ...
        var _data = this.__apopt.cache;
        _data.data.uid = this.__host.id;
        this.__apopt.item.host = this.__host;
        _data.lkey = 'apply-group-'+this.__host.id;
        this.__myap = _y._$$ListModuleWF._$allocate(this.__apopt);
    };
    /**
     * 控件初始化
     * @return {Void}
    ...
    _proModuleGroup.__doBuild = function(){
        ...
        this.__apopt = {
            ...
            onemptylist:function(_event){
                _event.value = '<p class="w-message">没有未审核的组织！</p>';
            }
        };
    };
};
