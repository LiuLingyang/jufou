...
<#macro groupHomeTop group={}>
  <@module id="6" class="s-srd">
    ...
    <div class="d fc07">
      <#noescape>
      <p class="fc05">${(group.description!"")?replace("\n","<br/>")}</p>
      </#noescape>
    </div>
  </@module>
</#macro>
...
<#macro groupHomeMain relation=0>
  <div class="l-rmc">
    <@module id="8" class="t-mdc">
      <#local items=["即将开始(${upcomingCount!0})","已经结束(${pastCount!0})"]>
      <#if relation==3>
        <#local items=["即将开始(${upcomingCount!0})","已经结束(${pastCount!0})","草稿箱"]>
      </#if>
...
<#macro groupHomeTemplate group={} meeting={} relation=0>
  ...
  <textarea name="txt" id="txt-home-nomeet">
    <#if relation==3>
    <div class="epty">
      <p class="tip fc06">您的组织里还没有发起过一次活动</p>
      ...
      <div class="w-meet bd00">
        ...
        <div class="sum f-cb">
          ...
          <div class="dtl bd00">
            ...
            <div class="fc05">
              <div class="js-flag smr"></div>
              <div class="mor bg00"><a class="fc02 js-flag" href="#"></a></div>
            </div>
  ...
  <textarea name="jst" id="jst-meeting-list">
    {list beg..end as y}
      ...
      <div class="w-meeting bd00">
        ...
        {/list}
        {list atd.length..4 as y}
          <img class="w-rd5p" src="${config.url.portrait}"/>
        {/list}
        {/trim}
        <div class="smr fc05">${x.details}</div>
      </div>
    {/list}
  </textarea>
  ...
  <textarea name="jst" id="jst-meeting-draft">
    {list beg..end as y}
      ...
      <div class="w-mdft f-cb bd00" id="${x.id|seed}">
        <a href="#" class="act fr fc02" data-action="delete" data-id="${x.id}">删除</a>
        <a href="${config.page()}/meeting/update/${x.id}/" class="act fr fc02">编辑</a>
        <div class="gtl"><a href="/${x.group.homepage}/meeting/${x.id}/" class="fc02">${x.title}</a></div>
      </div>
    {/list}
  </textarea>
